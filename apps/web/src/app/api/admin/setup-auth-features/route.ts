...
export async function POST() {
  try {
    ...
    if (connectorsResponse.ok) {
      ...
      hasEmailConnector = connectors.some((c: any) => 
        c.type === 'Email' || c.metadata?.type === 'Email'
      )
    ...
    if (hasEmailConnector) {
      const hasEmailMethod = updatedSignInExp.signIn.methods.some((m: any) => m.identifier === 'email')
      if (hasEmailMethod) {
    ...
    if (updateResponse.ok) {
      ...
      results.features.push({
        ...
        status: 'enabled',
        description: 'Users can enable TOTP (Google Authenticator), Passkeys (WebAuthn), or backup codes'
      })
    } else {
